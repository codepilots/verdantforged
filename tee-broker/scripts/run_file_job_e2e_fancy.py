...
SLIDES = [
    ...
    },
    {
        ...
        "bullets": [
            ...
            "Zero-Trust execution environment for any agent.",
    ...
    },
    {
        ...
        "bullets": [
            "AMD SEV-SNP enclaves ensure silicon-level trust.",
            "SHA-384 measurement of initial register state + runtime.",
            "Remote attestation: verified against AMD root cert chain.",
            "Proof of integrity: 1184-byte SNP report validation."
        ]
    },
    {
        ...
        "bullets": [
            ...
            "Broker never holds plaintext keys; decryption happens inside enclave.",
            "AAD-bound secrets (verdantforged-input) prevent data splicing.",
            "No debug interface in measured state."
        ]
    },
    {
        "title": "cat pillar_sandboxing.md",
        "bullets": [
            "Fresh, ephemeral WASM enclaves per workload.",
            "Default-DENY network policy (Broker LLM proxy only).",
            "Fuel-limited execution (max 10\u2079 cycles) prevents DoS.",
            "Strict 50MB per-file / 100MB total output caps."
        ]
    },
    {
        "title": "cat pillar_payment.md",
        "bullets": [
            "Stripe PaymentIntent verify-then-capture lifecycle.",
            "Funds held on requester side (requires_capture).",
            "No money moves until worker signs valid result envelope.",
    ...
    },
    {
        ...
        "bullets": [
            ...
            "Deploy Enclave: Attested m6a.xlarge, SEV-SNP root.",
            "Execute: NemoClaw sandbox, fuel-limited WASM.",
    ...
    },
    {
        ...
        "bullets": [
            ...
            "Secure Payments: Stripe PaymentIntents",
    ...
    },
    {
        ...
        "bullets": [
            ...
            "[ STEP 1 ] Compile & Publish WASM Skill.",
            "[ STEP 2 ] Submit encrypted payload to /v1/jobs.",
            "[ STEP 3 ] Worker enclave computes & signs output."
        ]
    },
    {
...
def run_e2e_job(args: argparse.Namespace, prompt: str) -> None:
    ...
    try:
        ...
        state.snp_quote_present = bool(att.get("snp_quote"))
    ...
    while True:
        ...
        if ch == ord('q') or ch == ord('Q'):
            ...
            if ch == ord(' ') or ch == 10 or ch == curses.KEY_RIGHT:  # Space, Enter or Right Arrow
                if state.current_slide == 9:  # Slide 10: Live Demo starts job
                    state.stage = "running"
                    ...
                    state.current_slide = min(11, state.current_slide + 1)
...
def render_presentation_slide(win: curses.window, width: int, height: int):
    ...
    elapsed_chars = int((time.time() - state.slide_start_time) * 35)
    ...
    if state.current_slide == 9 and len(typed_lines) == len(slide_lines):
...
def render_running_slide(win: curses.window, width: int, height: int):
    ...
    win.addstr(12, 4, f"[{bar}] {pct}%", curses.color_pair(2))
...
def render_result_slides(win: curses.window, width: int, height: int):
    ...
    if state.current_result_slide == 0:
        ...
        win.addstr(12, 2, "Demo Complete! Press [SPACE] to continue", curses.color_pair(5) | curses.A_BOLD)
    ...
    if state.current_result_slide < total_slides - 1:
        ...
        nav = " [<-] Prev | Continue [->] "
    win.addstr(13, (width - len(nav)) // 2, nav, curses.color_pair(1))
...
def render_error_slide(win: curses.window, width: int, height: int):
    ...
    win.addstr(13, (width - len(exit_msg)) // 2, exit_msg, curses.A_REVERSE | curses.color_pair(3))
